...
An example of a supervised regression femi.
...
def make_plots(df_clean: pd.DataFrame, model: LinearRegression) -> None:
    """Create and save charts for the supervised regression femi project."""
    ...
    scatter_plt.set_title("Hours Studied vs Score")
    scatter_plt.set_xlabel("Hours Studied")
    scatter_plt.set_ylabel("Score")

    hours_chart_path = IMAGE_DIR / "hours_studied_vs_score_femi.png"
    fig.tight_layout()
    fig.savefig(hours_chart_path)
    LOG.info(f"Saved chart: {hours_chart_path}")

    LOG.info("Creating chart: prior score vs score")

    fig, ax = plt.subplots(figsize=(9, 5))

    prior_score_plt: Axes = sns.scatterplot(
        data=df_clean,
        x="prior_score",
        y=TARGET_COL,
        ax=ax,
    ...
    prior_score_plt.set_title("Prior Score vs Final Score")
    prior_score_plt.set_xlabel("Prior Score")
    prior_score_plt.set_ylabel("Final Score")

    prior_score_chart_path = IMAGE_DIR / "prior_score_vs_score_femi.png"
    fig.tight_layout()
    fig.savefig(prior_score_chart_path)
    LOG.info(f"Saved chart: {prior_score_chart_path}")
    ...
    coefficients_chart_path = IMAGE_DIR / "model_coefficients_femi.png"
    fig.tight_layout()
    fig.savefig(coefficients_chart_path)
    LOG.info(f"Saved chart: {coefficients_chart_path}")
